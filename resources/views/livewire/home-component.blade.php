<main id="main">
    <div class="container">
        ...
        <!--On Sale-->
        @if($productsOnSale->count() > 0 && $saleTimeRecord->status == 1 && $saleTimeRecord->sale_date > \Carbon\Carbon::now())
        <div class="wrap-show-advance-info-box style-1 has-countdown">
            <h3 class="title-box">On Sale</h3>
            <div class="wrap-countdown mercado-countdown" data-expire="{{\Carbon\Carbon::parse($saleTimeRecord->sale_date)->format('Y/m/d h:m:s')}}"></div>
                 ...
                 @foreach($productsOnSale as $productOnSale)
                    <div class="product product-style-2 equal-elem ">
                        <div class="product-thumnail">
                            <a href="{{route('product.details',['slug'=>$productOnSale->slug])}}" title="{{$productOnSale->name}}">
                                <figure><img src="{{asset('assets/images/products')}}/{{$productOnSale->image}}" width="800"
                        ...
                        <div class="product-info">
                            <a href="{{route('product.details',['slug'=>$productOnSale->slug])}}"
                               class="product-name"><span>{{$productOnSale->name}}</span></a>
                            <div class="onSaleProductsPriceContainer wrap-price">
                                <p class="product-price"><ins>{{$productOnSale->sale_price}}$</ins> </p>
                                <p class="product-price"><del>{{$productOnSale->regular_price}}$</del> </p>
                            </div>
                        </div>
                    </div>
                @endforeach
                        ...
                        <div class="tab-content-item {{$key == 0 ? 'active' : ''}}" id="category_{{$category->id}}">
                            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container"
                                 ...
                                 data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}'>
                                    ...
                                    @php
                                        $c_products = DB::table("products")->where("category_id",$category->id)->get()->take($numberOfProducts);
                                    @endphp
